...
import org.devzendo.morsetrainer.gui.dialogs.{HelpAboutDialog, ToolsOptionsDialog}

class HelpAboutActionListener(mainFrame: Frame, cursorManager: CursorManager) extends SnailActionListener(cursorManager) {
    def actionPerformedSlowly(e: ActionEvent) {
        GUIUtils.invokeLaterOnEventThread(new Runnable() {
            def run() {
                val dialog = new HelpAboutDialog(mainFrame, cursorManager)
